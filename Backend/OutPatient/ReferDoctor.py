    ...
    if request.method == 'POST':
        try:
            ...
            for favourite in favourites_arr:
                ...
                test_codearr = favourite.get('TestDetails', [])
                
                fav_ins = TestName_Favourites.objects.get(Favourite_Code=favourite_key)
                for test_code in test_codearr:
                    indivitualcode = test_code.get('TestCode', '')
                    test_Code_ins = LabTestName_Details.objects.get(TestCode=indivitualcode)
                    Lab_Request_Details.objects.create(
                        TestType=test_type,
                        FavouriteCode=fav_ins,
                        IndivitualCode=test_Code_ins,
                        created_by=created_by,
                        **register_kwargs
                    )
